        ...
        foreach ($stig in $jsonContent.stigs) {
            ...
            foreach ($rule in $stig.rules) {
                ...
                $vulnData = @{
                    ...
                    Discussion = $rule.discussion.replace("…","&hellip;")
                    CheckContent = $rule.check_content.replace("…","&hellip;")
                    FixText = $rule.fix_text.replace("…","&hellip;")
...
"@
        ...
        foreach ($vuln in $openVulns) {
            ...
            $statusBadge = "<strong class='severity-$severityClass'>[$severityLabel]</strong>"
            
            # Create link with status badge at the beginning
            $html += "                                <a onclick=`"showFindingModalById('$findingId')`" class='rule-link'>$statusBadge $($vuln.VulnID) - $($vuln.RuleTitle)</a>`n"
        }
...
"@
        ...
        $notReviewedVulns = $checklist.Vulns | Where-Object { $_.Status -eq "Not_Reviewed" } | Sort-Object VulnID
        
        foreach ($vuln in $notReviewedVulns) {
            $findingId = "not_reviewed-$($checklist.HostName)-$($vuln.VulnID)" -replace '\s','-'
...
if ($totalOpen -eq 0 -and $totalNotReviewed -eq 0) {
    ...
    if ($totalHighOpen -eq 0) {
        ...
        foreach ($checklist in $allChecklists) {
            $cat1Findings = $checklist.Vulns | Where-Object { $_.Status -eq "Open" -and $_.Severity -eq "high" } | Sort-Object VulnID
            ...
            if ($cat1Findings.Count -gt 0) {
                ...
                foreach ($finding in $cat1Findings) {
                    ...
                    # Escape data for JSON in data attributes
                    $findingDataJson = @{
                    ...
                    $escapedJson = [System.Web.HttpUtility]::HtmlAttributeEncode($findingDataJson)
                    
                    $html += @"
                    <div class="finding-card severity-high" id="$findingId" data-finding='$escapedJson' onclick="showFindingModal(this)" style="cursor: pointer;">
                        ...
                        <div class="finding-details">
                            <strong>Host:</strong> $($checklist.HostName) ($($checklist.FileName))
                        </div>
"@
...
"@
    ...
    if ($totalMediumOpen -eq 0) {
        ...
        $checklistGroupIndex = 0
        foreach ($checklist in $allChecklists) {
            $cat2Findings = $checklist.Vulns | Where-Object { $_.Status -eq "Open" -and $_.Severity -eq "medium" } | Sort-Object VulnID
            ...
            if ($cat2Findings.Count -gt 0) {
                ...
                foreach ($finding in $cat2Findings) {
                    ...
                    $html += @"
                    <div class="finding-card severity-medium" id="$findingId" data-finding='$escapedJson' onclick="showFindingModal(this)" style="cursor: pointer;">
                        <div class="finding-header">
                            <div class="finding-title">$($finding.VulnID): $($finding.RuleTitle)</div>
...
"@
    ...
    if ($totalLowOpen -eq 0) {
        ...
        $checklistGroupIndex = 0
        foreach ($checklist in $allChecklists) {
            $cat3Findings = $checklist.Vulns | Where-Object { $_.Status -eq "Open" -and $_.Severity -eq "low" } | Sort-Object VulnID
            ...
            if ($cat3Findings.Count -gt 0) {
                ...
                foreach ($finding in $cat3Findings) {
                    ...
                    $html += @"
                    <div class="finding-card severity-low" id="$findingId" data-finding='$escapedJson' onclick="showFindingModal(this)" style="cursor: pointer;">
                        <div class="finding-header">
                            <div class="finding-title">$($finding.VulnID): $($finding.RuleTitle)</div>
...
"@
    ...
    if ($totalNotReviewed -eq 0) {
        ...
        $checklistGroupIndex = 0
        foreach ($checklist in $allChecklists) {
            $notReviewedItems = $checklist.Vulns | Where-Object { $_.Status -eq "Not_Reviewed" } | Sort-Object VulnID
            ...
            if ($notReviewedItems.Count -gt 0) {
                ...
                foreach ($item in $notReviewedItems) {
                    $findingId = "not_reviewed-$($checklist.HostName)-$($item.VulnID)" -replace '\s','-'
                    ...
                    $html += @"
                    <div class="finding-card" id="$findingId" data-finding='$escapedJson' onclick="showFindingModal(this)" style="cursor: pointer;">
                        <div class="finding-header">
...
$html += @"
        <div class="section-wrapper">
            <div class="section-header" onclick="toggleSection('unique-findings')">
                <h2>Unique Open & Not Reviewed Findings by STIG</h2>
                <span class="collapse-icon">&#9660;</span>
            </div>
            <div class="section-body" id="unique-findings">
"@

# Group all vulnerabilities by VulnID and STIG (Open and Not Reviewed)
$uniqueFindings = @{}

foreach ($checklist in $allChecklists) {
    foreach ($vuln in $checklist.Vulns) {
        if ($vuln.Status -eq "Open" -or $vuln.Status -eq "Not_Reviewed") {
            $key = "$($vuln.STIGTitle)|$($vuln.VulnID)|$($vuln.Status)"
...
"@
    ...
    foreach ($finding in $findings) {
        ...
        $statusBadge = ""
        if ($finding.Status -eq "Open") {
            $statusBadge = "<strong class='severity-$severityClass'>[$severityLabel]</strong>"
        } elseif ($finding.Status -eq "Not_Reviewed") {
            $statusBadge = "<span class='status-notreviewed'>[Not Reviewed]</span>"
        }
        ...
        $html += @"
                <div class="collapsible-section" style="margin: 10px 0;">
                    <div class="nested-finding-header" onclick="toggleCollapsible('finding-$stigGroupIndex-$findingIndex')">
                        <div>
                            $statusBadge $($finding.VulnID): $($finding.RuleTitle)
                        </div>
                        <span class="collapse-icon">&#9660;</span>
                    </div>
...
"@
        ...
        foreach ($hostInfo in $finding.Hosts) {
            # Construct the finding ID to reference the finding in the All Open Findings or Not Reviewed tab
            $statusPrefix = if ($finding.Status -eq "Open") { "open" } else { "not_reviewed" }
            $findingId = "$statusPrefix-$($hostInfo.HostName)-$($finding.VulnID)" -replace '\s','-'
            
            $html += "                            <div class='host-item' style='cursor: pointer; color: #3498db;' onclick=`"showFindingModalById('$findingId')`"><strong>$($hostInfo.HostName)</strong> ($($hostInfo.HostIP)) - $($hostInfo.FileName)</div>`n"
        }
...
$html += @"
    ...
    <!-- Modal for displaying finding details -->
...
$html += @"
    ...
    <script>
        function showFindingModal(element) {
            // Get finding data from element's data attributes
            var findingJson = element.getAttribute('data-finding');
            if (!findingJson) {
                console.error('Finding data not found');
                return;
            }
            ...
            modalTitle.innerHTML = finding.vulnId + ': ' + finding.ruleTitle;
            
            var severityClass = 'severity-' + finding.severityClass;
            var statusClass = 'status-' + finding.status.toLowerCase().replace('_', '');
            
            var bodyContent = '<div style="margin-bottom: 20px;">';
            bodyContent += '<div class="modal-field"><span class="modal-field-label">Severity:</span> <span class="' + severityClass + '">' + finding.severity + '</span></div>';
            bodyContent += '<div class="modal-field"><span class="modal-field-label">Status:</span> <span class="' + statusClass + '">' + finding.status.replace('_', ' ') + '</span></div>';
            bodyContent += '<div class="modal-field"><span class="modal-field-label">Rule ID:</span> <span class="modal-field-value">' + finding.ruleId + '</span></div>';
            bodyContent += '<div class="modal-field"><span class="modal-field-label">STIG:</span> <span class="modal-field-value">' + finding.stigTitle + '</span></div>';
            bodyContent += '<div class="modal-field"><span class="modal-field-label">Group:</span> <span class="modal-field-value">' + finding.groupTitle + '</span></div>';
            bodyContent += '<div class="modal-field"><span class="modal-field-label">Host:</span> <span class="modal-field-value">' + finding.hostName + ' (' + finding.fileName + ')</span></div>';
            
            if (finding.findingDetails) {
                bodyContent += '<div class="modal-field"><div class="modal-field-label">Finding Details:</div><div class="modal-field-value">' + finding.findingDetails.replace(/\n/g, '<br>') + '</div></div>';
            }
            
            if (finding.comments) {
                bodyContent += '<div class="modal-field"><div class="modal-field-label">Comments:</div><div class="modal-field-value">' + finding.comments.replace(/\n/g, '<br>') + '</div></div>';
            }
            
            if (finding.discussion) {
                bodyContent += '<div class="modal-field"><div class="modal-field-label">Discussion:</div><div class="modal-field-value">' + finding.discussion.replace(/\n/g, '<br>') + '</div></div>';
            }
            
            if (finding.checkContent) {
                bodyContent += '<div class="modal-field"><div class="modal-field-label">Check Content:</div><div class="modal-field-value">' + finding.checkContent.replace(/\n/g, '<br>') + '</div></div>';
            }
        ...
        function showFindingModalById(findingId) {
            // Find the element with this ID
            var element = document.getElementById(findingId);
            if (!element) {
                console.error('Finding element not found:', findingId);
                return;
            }
            // Call the main modal function with the element
            showFindingModal(element);
        }
