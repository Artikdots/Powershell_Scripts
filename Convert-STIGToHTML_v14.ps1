...
"@
        ...
        foreach ($vuln in $openVulns) {
            ...
            $html += "                                <a onclick=`"showFindingModal('$findingId')`" class='rule-link'>$($vuln.VulnID) - $($vuln.RuleTitle) [$severityLabel]</a>`n"
        }
...
"@
        ...
        foreach ($vuln in $notReviewedVulns) {
            ...
            $html += "                                <a onclick=`"showFindingModal('$findingId')`" class='rule-link'>$($vuln.VulnID) - $($vuln.RuleTitle)</a>`n"
        }
                ...
                foreach ($finding in $cat1Findings) {
                    ...
                    <div class="finding-card severity-high" id="$findingId">
                        ...
                        <div class="finding-details">
                            <span class="finding-title"><strong>Rule ID:</strong></span> $($finding.RuleID)
                        </div>
                        <div class="finding-details">
                            <span class="finding-title"><strong>STIG:</strong></span> $($finding.STIGTitle)
                        </div>
                        <div class="finding-details">
                            <span class="finding-title"><strong>Title:</strong></span> $($finding.GroupTitle)
                        </div>
"@
                    if ($finding.FindingDetails) {
                        $html += @"
                        <div class="finding-details">
                            <span class="finding-title"><strong>Finding Details:</strong></span><br>
...
"@
                    ...
                    if ($finding.Comments) {
                        $html += @"
                        <div class="finding-details">
                            <span class="finding-title"><strong>Comments:</strong></span><br>
...
$html += @"
    ...
    <script>
        // Finding data for modal display
        var findingData = {};
"@

# Build the finding data object
foreach ($checklist in $allChecklists) {
    foreach ($vuln in $checklist.Vulns) {
        $findingId = "$($vuln.Status.ToLower())-$($checklist.HostName)-$($vuln.VulnID)" -replace '\s','-'
        $severityLabel = switch ($vuln.Severity) {
            "high" { "CAT I" }
            "medium" { "CAT II" }
            "low" { "CAT III" }
        ...
        $ruleTitle = $vuln.RuleTitle -replace "'", "\'" -replace '"', '\"' -replace "`n", "\n" -replace "`r", ""
        $ruleID = $vuln.RuleID -replace "'", "\'" -replace '"', '\"'
        $stigTitle = $vuln.STIGTitle -replace "'", "\'" -replace '"', '\"'
        $groupTitle = $vuln.GroupTitle -replace "'", "\'" -replace '"', '\"'
        $findingDetails = if ($vuln.FindingDetails) { $vuln.FindingDetails -replace "'", "\'" -replace '"', '\"' -replace "`n", "\n" -replace "`r", "" } else { "" }
        $comments = if ($vuln.Comments) { $vuln.Comments -replace "'", "\'" -replace '"', '\"' -replace "`n", "\n" -replace "`r", "" } else { "" }
        $discussion = if ($vuln.Discussion) { $vuln.Discussion -replace "'", "\'" -replace '"', '\"' -replace "`n", "\n" -replace "`r", "" } else { "" }
        $checkContent = if ($vuln.CheckContent) { $vuln.CheckContent -replace "'", "\'" -replace '"', '\"' -replace "`n", "\n" -replace "`r", "" } else { "" }
...
$html += @"
        ...
        function showFindingModal(findingId) {
            var finding = findingData[findingId];
            if (!finding) {
