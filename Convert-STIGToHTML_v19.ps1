...
foreach ($stigTitle in ($stigGroups.Keys | Sort-Object)) {
    $findings = $stigGroups[$stigTitle] | Sort-Object { 
        # Primary sort by severity, secondary by status
        $severityOrder = switch ($_.Severity) {
            "high" { 1 }
            "medium" { 2 }
            "low" { 3 }
            default { 4 }
        ...
        $statusOrder = switch ($_.Status) {
            "Open" { 0 }
            "Not_Reviewed" { 1 }
            default { 2 }
        }
        
        # Combine: multiply severity by 10 and add status to ensure proper ordering
        ($severityOrder * 10) + $statusOrder
    }
