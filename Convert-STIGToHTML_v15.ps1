...
"@
        ...
        foreach ($vuln in $openVulns) {
            ...
            # Create JSON object for data attribute
            $findingJson = @{
                vulnId = $vuln.VulnID
                ruleId = $vuln.RuleID
                ...
                status = $vuln.Status
                findingDetails = if ($vuln.FindingDetails) { $vuln.FindingDetails } else { "" }
                comments = if ($vuln.Comments) { $vuln.Comments } else { "" }
                discussion = if ($vuln.Discussion) { $vuln.Discussion } else { "" }
                checkContent = if ($vuln.CheckContent) { $vuln.CheckContent } else { "" }
                hostName = $checklist.HostName
                fileName = $checklist.FileName
            } | ConvertTo-Json -Compress
            
            # Escape for HTML attribute
            $findingJsonEscaped = [System.Web.HttpUtility]::HtmlAttributeEncode($findingJson)
            
            $html += "                                <a onclick=`"showFindingModal(this); return false;`" class='rule-link' data-finding=`"$findingJsonEscaped`">$($vuln.VulnID) - $($vuln.RuleTitle) [$severityLabel]</a>`n"
        }
...
"@
        ...
        foreach ($vuln in $notReviewedVulns) {
            ...
            $findingJson = @{
                ...
                ruleId = $vuln.RuleID
                severity = "N/A"
                severityClass = if ($vuln.Severity) { $vuln.Severity } else { "medium" }
                ruleTitle = $vuln.RuleTitle
                stigTitle = $vuln.STIGTitle
                groupTitle = $vuln.GroupTitle
                status = $vuln.Status
            ...
            $html += "                                <a onclick=`"showFindingModal(this); return false;`" class='rule-link' data-finding=`"$findingJsonEscaped`">$($vuln.VulnID) - $($vuln.RuleTitle)</a>`n"
        }
        ...
        foreach ($checklist in $allChecklists) {
            ...
            if ($cat1Findings.Count -gt 0) {
                ...
                foreach ($finding in $cat1Findings) {
                    ...
                    $html += @"
                    <div class="finding-card severity-high finding-summary" id="$findingId" data-finding='$escapedJson' onclick="showFindingModal(this)">
        ...
        foreach ($checklist in $allChecklists) {
            ...
            if ($cat2Findings.Count -gt 0) {
                ...
                foreach ($finding in $cat2Findings) {
                    ...
                    $html += @"
                    <div class="finding-card severity-medium finding-summary" id="$findingId" data-finding='$escapedJson' onclick="showFindingModal(this)">
                        <div class="finding-header">
                            <div class="finding-title">$($finding.VulnID): $($finding.RuleTitle)</div>
        ...
        foreach ($checklist in $allChecklists) {
            ...
            if ($cat3Findings.Count -gt 0) {
                ...
                foreach ($finding in $cat3Findings) {
                    ...
                    $html += @"
                    <div class="finding-card severity-low finding-summary" id="$findingId" data-finding='$escapedJson' onclick="showFindingModal(this)">
                        <div class="finding-header">
                            <div class="finding-title">$($finding.VulnID): $($finding.RuleTitle)</div>
        ...
        foreach ($checklist in $allChecklists) {
            ...
            if ($notReviewedItems.Count -gt 0) {
                ...
                foreach ($item in $notReviewedItems) {
                    ...
                    $html += @"
                    <div class="finding-card finding-summary" id="$findingId" data-finding='$escapedJson' onclick="showFindingModal(this)">
                        <div class="finding-header">
