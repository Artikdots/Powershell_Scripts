    ...
    try {
        ...
        foreach ($stig in $jsonContent.stigs) {
            ...
            foreach ($rule in $stig.rules) {
                ...
                # Check for severity overrides
                $severity = $rule.severity
                if ($rule.overrides -and $rule.overrides.severity) {
...
# Group checklists by STIG Title manually since they're hashtables
$stigSummary = @{}

foreach ($checklist in $allChecklists) {
    $stigTitle = $checklist.STIGTitle
    ...
    $openVulns = $checklist.Vulns | Where-Object { $_.Status -eq "Open" }
    $stigSummary[$stigTitle].CatI += ($openVulns | Where-Object { $_.Severity -eq "high" }).Count
    $stigSummary[$stigTitle].CatII += ($openVulns | Where-Object { $_.Severity -eq "medium" }).Count
    $stigSummary[$stigTitle].CatIII += ($openVulns | Where-Object { $_.Severity -eq "low" }).Count
}
    ...
    if ($totalHighOpen -eq 0) {
        ...
        foreach ($checklist in $allChecklists) {
            ...
            if ($cat1Findings.Count -gt 0) {
                ...
                $html += "<h3>$($checklist.FileName) - $($checklist.HostName)</h3>"
                ...
                foreach ($finding in $cat1Findings) {
                    ...
                    <div class="finding-card severity-high" id="$findingId">
                        <div class="finding-header">
                            ...
                            <span class="severity-high" style="white-space: nowrap;">CAT I</span>
                        </div>
            ...
            if ($cat2Findings.Count -gt 0) {
                ...
                foreach ($finding in $cat2Findings) {
                    ...
                    <div class="finding-card severity-medium" id="$findingId">
                        <div class="finding-header">
                            <div class="finding-title">$($finding.VulnID): $($finding.RuleTitle)</div>
                            <span class="severity-medium" style="white-space: nowrap;">CAT II</span>
                        </div>
                        <div class="finding-details">
                            <strong>Rule ID:</strong> $($finding.RuleID)
                        </div>
            ...
            if ($cat3Findings.Count -gt 0) {
                ...
                foreach ($finding in $cat3Findings) {
                    ...
                    <div class="finding-card severity-low" id="$findingId">
                        <div class="finding-header">
                            <div class="finding-title">$($finding.VulnID): $($finding.RuleTitle)</div>
                            <span class="severity-low" style="white-space: nowrap;">CAT III</span>
                        </div>
                        <div class="finding-details">
                            <strong>Rule ID:</strong> $($finding.RuleID)
                        </div>
                ...
                foreach ($item in $notReviewedItems) {
                    ...
                    <div class="finding-card" id="$findingId">
                        <div class="finding-header">
                            ...
                            <span class="status-notreviewed" style="white-space: nowrap;">Not Reviewed</span>
                        </div>
                        <div class="finding-details">
        ...
        $html += @"
                <div class="collapsible-section" style="margin: 10px 0;">
                    <div class="nested-finding-header" onclick="toggleCollapsible('finding-$stigGroupIndex-$findingIndex')">
                        <div>
                            <strong class="severity-$severityClass">[$severityLabel]</strong> $($finding.VulnID): $($finding.RuleTitle) ($($finding.Hosts.Count))
                        </div>
                        <span class="collapse-icon">&#9660;</span>
                    </div>
...
Write-Host "  Pass Rate: $($passPercent + $naPercent)%"
